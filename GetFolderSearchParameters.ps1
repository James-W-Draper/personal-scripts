...
if ($addressOrSite.IndexOf("@") -ige 0)
{
   ...
   if (!$ExoSession)
   {
       ...
       Connect-ExchangeOnline -ShowBanner:$false -CommandName Get-MailboxFolderStatistics -resultsize unlimited
   }
   $folderQueries = @()
   $folderStatistics = Get-MailboxFolderStatistics $emailAddress -resultsize unlimited
